...
import lambdanet.PrepareRepos.{ParsedRepos, hiddenNodes}
...
object TypeInferenceService {
  ...
  def loadModel(
    ...
    if (amm.exists(modelCachePath)) {
      announced("Loading model from cache") {
        println("This process can take from 1-3 minutes to complete.")
  ...
  case class PredictionResults(
    ...
    def prettyPrint(): Unit = {
      ...
      byFile.foreach {
        case (file, nodes) =>
          val inputLines = nodes.toSeq.sortBy(_.srcSpan.get.start).length
          val hiddenVar = hiddenNodes
          val outputs = inputLines*10
          val trainableParameters = (hiddenVar*hiddenVar) + hiddenVar + (hiddenVar*inputLines) + inputLines + (hiddenVar*outputs) + outputs
          println("Number of Trainable Parameters: " + trainableParameters)
          println(s"=== Filename: $file ===")
          println("(Line, Column) : [Ranking] (Certainty) Variable Type:")
