...
object ProgramParsing {
  ...
  def parseGModulesFromFiles(
    ...
    SimpleMath.withErrorMessage {
      //Windows
      val p = "C:/cliInference/"
      //Linux
      //val p = "/usr/local/cliInference/Resources"
      val command = s"node ${absPath(p) / RelPath("scripts/ts/parsingFromFile.js")} " +
      ...
      val r = %%(
        'node,
        absPath(p) / RelPath("scripts/ts/parsingFromFile.js"),
        "--src",
...
case class ProgramParsing() {
  ...
  def parseContent(content: String): Vector[GStmt] = {
    SimpleMath.withErrorMessage(
      ...
      //Linux
      //val p = "/usr/local/cliInference/Resources"
      val r = %%('node, "./parsingFromString.js", content)(absPath(p)/ 'scripts / 'ts)
